...
from domainbed.lib.fast_data_loader import InfiniteDataLoader, FastDataLoader, DCTDataLoader, visualDataLoader
...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    # setup loaders
    datas = [env for env,_ in iterator.train(in_splits)]
    if hparams["sampler"]=='DCT':
        train_loaders = [DCTDataLoader(
                dataset=datas,
                batch_size=hparams["DCT_batch_size"],
                num_workers=dataset.N_WORKERS,
                classnum=dataset.num_classes,
                domainnum=n_envs-1
            )]
    ...
    visDataLoader = [visualDataLoader(
        dataset=datas,
        batch_size=64,
        num_workers=dataset.N_WORKERS,
        envi=args.test_envs[0]
    )]
    # setup eval loaders
    eval_loaders_kwargs = []
    ...
    algorithm.to(device)
    if args.algorithm=="DCTP":
        algorithm.setall(visDataLoader)
    ...
    # summary(algorithm,(3,224,224),32)
    ...
    for step in range(n_steps):
        ...
        # if "DCT_batch_size" in hparams.keys():
        #     steps_per_epoch = train_loaders[0].sampler.length//hparams["DCT_batch_size"]
            # print(steps_per_epoch)
        elif hparams["sampler"]=='DCT':
            x,y,dmy = batches_dictlist[0]
            x = x.to(device)
            y = y.to(device)
            dmy = dmy.to(device)
        # print(inputs)
    ...
    if args.dataset!='DomainNet':
        misc.visualization(algorithm, visDataLoader, device, args.out_dir,test_envs[0])
    # find best
    logger.info("---")
    records = Q(records)
