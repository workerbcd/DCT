...
class _SplitDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, underlying_dataset, keys,test_env=None):
        super(_SplitDataset, self).__init__()
        self.underlying_dataset = underlying_dataset
        self.keys = keys
        self.transforms = {}
        assert len(test_env)==1, "only leave one test domain"
        self.test_env = test_env[0]
    ...
    def __getitem__(self, key):
        ...
        x, y,dmy = self.underlying_dataset[self.keys[key]]
        if dmy>self.test_env: dmy-=1
        ret = {"y": y,"dmy": dmy}
...
def split_dataset(dataset, n, seed=0 ,test_env=None):
    """
    ...
    keys_1 = keys[:n]
    keys_2 = keys[n:]
    return _SplitDataset(dataset, keys_1,test_env=test_env), _SplitDataset(dataset, keys_2,test_env=test_env)
