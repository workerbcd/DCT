...
from domainbed.losses.tripletloss import *
...
def accuracy_from_loader(algorithm, loader, weights, debug=False):
    ...
    losssum = 0.0
    weights_offset = 0
    tripletloss = TripletLoss(margin=12)
    ...
    for i, batch in enumerate(loader):
        ...
        with torch.no_grad():
            feat = algorithm.get_feats(x)
            logits = algorithm.predict(x)
            loss = F.cross_entropy(logits, y).item()+tripletloss(feat,y,-1)[0].item()
