...
from domainbed.optimizers import get_optimizer,get_optimizer_DCT
...
from domainbed.losses.tripletloss import TripletLoss,DomainTripletLoss
from domainbed.losses.DCTP import DomainTripletLoss_Prototype
from domainbed.losses.PCLloss import PCLLoss, ProxyPLoss, ProxyDCLoss
from domainbed.algorithms.msl_layer import *
...
class Algorithm(torch.nn.Module):
    ...
    def update(self, x, y,dmy=None, **kwargs):
        """
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        hparams["out_dim"] = self.featurizer.n_outputs
...
class AbstractMMD(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, gaussian):
        ...
        self.use_loss_bn = not hparams["use_bnn"]
        self.hparams=hparams
    ...
    def update(self, x, y,dmy=None, **kwargs):
        if isinstance(dmy, list):
            all_dmy = torch.cat(dmy)
        else:
            all_dmy = dmy
        minibatches = to_minibatch(x, y)
        objective = 0
        ...
        if nmb > 1:
            penalty /= nmb * (nmb - 1) / 2
        loss_dct = self.DCTloss(torch.cat(features), torch.cat(targets),all_dmy,
                                normalize_feature=self.hparams["normfeat"], use_bn=self.use_loss_bn)[0]
        self.optimizer.zero_grad()
        (loss_dct+objective + (self.hparams["mmd_gamma"] * penalty)).backward()
        self.optimizer.step()

        if torch.is_tensor(penalty):
            penalty = penalty.item()

        return {"loss": objective.item(), "penalty": penalty, "dct": loss_dct.item()}
...
class RSC(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.num_classes = num_classes
        hparams["out_dim"] = self.featurizer.n_outputs
        self.DCT = DomainTripletLoss(hparams,margin=hparams["margin"])
    ...
    def update(self, x, y,dmy=None, **kwargs):
        ...
        # Equation (5): update
        celoss = F.cross_entropy(all_p_muted_again, all_y)
        dctloss = self.DCT(all_f,all_y,all_dmy,use_bn=True)[0]
        loss = celoss+dctloss
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()

        return {"celoss": celoss.item(), "dctloss": dctloss.item()}

class DCT(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DCT, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.featurizer = networks.Featurizer(input_shape, self.hparams,numclass=num_classes)
        # self._initialize_weights(self.featurizer)
        self.hparams['out_dim'] = self.featurizer.n_outputs
        # self.bottleneck = nn.BatchNorm1d(hparams["out_dim"])
        # self.bottleneck.bias.requires_grad_(False)
        # self.bottleneck.apply(networks.weights_init_kaiming)
        self.classifier = nn.Linear(self.featurizer.n_outputs, num_classes)
        self.classifier.apply(networks.weights_init_classifier)
        ...
        self.hparams = hparams
        if hparams["backbone"] == "swag_regnety_16gf":
            self.featurizer = torch.nn.DataParallel(self.featurizer,device_ids=[0,1])
            hparams["margin"]=0
        self.DCTloss = DomainTripletLoss(self.hparams, margin=hparams["margin"])
        print("notdomain")
        if not hparams["use_reidoptim"]:
    ...
    def update(self,x,y,dmy=None,**kwargs):
        ...
        if self.hparams["backbone"] == "resnet":
            ...
            lossDCT,_ = self.DCTloss(feats, all_y, all_dmy, normalize_feature=self.hparams["normfeat"],use_bn=self.use_loss_bn)
            scores = self.classifier(feats)

            lossCE = F.cross_entropy(scores, all_y)
        elif self.hparams["backbone"] == "swag_regnety_16gf":
            scores, feats = self.featurizer(all_x)
            lossDCT = self.DCTloss(feats, all_y, all_dmy, normalize_feature=True,use_bn=False)[0]
            lossCE = F.cross_entropy(scores, all_y)
        else:
            raise NotImplementedError(f"backbone is wrong")

        loss = lossCE+lossDCT
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        return {"lossCE": lossCE.item(), "lossDCT":lossDCT.item()}
    def make_optimizer(self):
        ...
        for key, value in self.named_parameters():
            ...
            if "bias" in key:
                ...
                weight_decay = self.hparams["weight_decay_bias"]

            params += [{"params": [value], "lr": lr, "weight_decay": weight_decay}]
        if self.hparams["optimizer"] == 'sgd':
            self.optimizer = get_optimizer_DCT(self.hparams["optimizer"], params, momentum=self.hparams["momentum"])
        else:
            self.optimizer = get_optimizer(self.hparams["optimizer"], params)
    def predict(self, x):
        if self.hparams["backbone"] == "resnet":
            feats = self.featurizer(x)
            scores = self.classifier(feats)
        elif self.hparams["backbone"] == "swag_regnety_16gf":
            scores, _ = self.featurizer(x)
        else:
            raise NotImplementedError(f"backbone is wrong")
        return scores
    def get_feats(self,x):
        if self.hparams["backbone"] == "resnet":
            ...
            _,feats = self.featurizer(x)
        else:
            raise NotImplementedError(f"backbone is wrong")
        return feats
class MSL(ERM):
    def __init__(self,input_shape, num_classes, num_domains, hparams,converge_step=3000):
        super(MSL, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.featsize = self.featurizer.n_outputs
        self.classifier = Mahalanobis(self.featsize,num_classes)
        self.network = nn.Sequential(self.featurizer, self.classifier)
